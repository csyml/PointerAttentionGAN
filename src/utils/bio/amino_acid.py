...
def filter_non_standard_amino_acids(data, column,filter_method="deletion"):
    """
    Args:
...
def numpy_seqs_to_fasta(sequences, id_to_enzyme_class, labels=None, d_scores=None, strip_zeros=False):
    ...
    return sequences_to_fasta(seqs, True, strip_zeros)


def sequences_to_fasta(sequences, escape=True, strip_zeros=False):
    """
    ...
    """
    return os.linesep.join([seq.get_seq_in_fasta(escape, strip_zeros) for seq in sequences])
...
def fasta_to_pandas(path, separator=";"):
    ...
    with open(path) as fasta_file:
        ...
        for title, sequence in SimpleFastaParser(fasta_file):
            ...
            if len(title_parts[0])==1: #only with id and description==null
                identifiers.append(int(title_parts[0]))  # First word is ID
            else: #the description of fasta file != null
                identifiers.append(title_parts[0])
